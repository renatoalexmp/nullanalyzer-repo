...
/**
 * Tracks which locals are definitely non-null. Author: Patrick Lam
 * (plam@sable.mcgill.ca) Based on BranchedRefVarsAnalysis by Janus Godard
 * (janus@place.org).
 */
public class NullnessAnalysis extends ForwardBranchedFlowAnalysis {
	...
	protected void flowThrough(Object srcValue, Unit unit, List fallOut,
			List branchOuts) {
		FlowSet dest;
		...
		if (unit instanceof IfStmt) {
			...
			// case 2: both ops are local and one op is non-null and testing
			// equality
	...
	private void addGensFor(DefinitionStmt u) {
		...
		if (ro instanceof NewExpr || ro instanceof NewArrayExpr
